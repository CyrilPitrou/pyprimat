 ...
 * pyprimat.background.StandardBackground/CustomBackground run with the
 * default PyPRConfig() (Omegabh2=0.022425, incomplete_decoupling=
...
static void test_standard(void)
{
    ...
    char *err = NULL;
    CPRConfig cfg;
    CHECK(cpr_config_init_defaults(&cfg, "../pyprimat", &err) == 0, "config init succeeds");
...
static void test_ede(void)
{
    char *err = NULL;
    CPRConfig cfg;
    cpr_config_init_defaults(&cfg, "../pyprimat", &err);
    cfg.fEDE = 0.3;
    cfg.zcEDE = 4.0e9;
    cfg.wnEDE = 3.0;
