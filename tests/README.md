...
| `test_constants.py` | `pyprimat.constants.CONST`'s derived electroweak values: `sW2` (sin²θ_W) against an independent hand-computation of the on-shell relation, the `geL`/`geR`/`gmuL` effective couplings derived from it, and `T_weak`/`T_nucl` against `MeV_to_Kelvin`. |
...
| `test_cache_utils.py` | The fingerprinted-cache helpers (`pyprimat.cache_utils`): `fingerprint_hash` is order-independent and value-sensitive; write/read round-trips; a missing file or a header-less/corrupt file is reported as unknown fingerprint (`None`) rather than raising; and that `write_cache_with_fingerprint` writes atomically (temp file + `os.replace`, no leftover `.tmp.<pid>` file, safe to overwrite an existing cache). |
