...
class PyPR:
    ...
    def _setup_background_and_cosmo(self):
        ...
        *Instantaneous (complete) decoupling* (``False``):
        ...
        # ------------------------------------------------------------------
        # Step 1 – Build neutrino-temperature functions and N_NEVO(T_γ)
        # ------------------------------------------------------------------
        if cfg.incomplete_decoupling:
            # Full non-instantaneous treatment: read NEVO table.
            ...
            # Instantaneous decoupling: all three flavours share the same
            # temperature derived from EM entropy conservation:
            #
            #     T_ν(T_γ) = [spl(T_γ) / sbar_ref]^{1/3}
            #
            # sbar_ref is the high-T limit of spl(T)/T³ (photons + e+e- at
            # T >> m_e), normalised so that T_ν = T_γ before e+e- annihilation.
            #
        ...
        # ------------------------------------------------------------------
        # Step 1b – Spectral distortions setup
        # ------------------------------------------------------------------
        # Build self._dFDneu_func: the deviation of the neutrino distribution
        # from a Fermi-Dirac at temperature T_ν, used as an additive correction
        # to the n<->p weak-rate integrand (see weak_rates.py, δχ).
        #
        # The function signature matches PRIMAT's dFDneu:
        #   dFDneu_func(en, x, znu, sgnq)  →  scalar
        # where en = E/me, x = me/(kB Tγ), znu = me/(kB Tν), sgnq = ±1.
        # Returns 0 when x is outside the valid temperature range.
        self._dFDneu_func = None  # None means "no spectral distortions"
        self._rho_nu_SD   = None  # None means "no extra energy density"
        ...
        if cfg.spectral_distortions:
            ...
            if cfg.analytic_distortions:
                ...
                def _fd(arg):
                    ...
                    return 0. if arg > exp_cut else 1. / (np.exp(arg) + 1.)
                ...
                def dFDneu_func(en, x, znu, sgnq):
                    ...
                    arg_nu  = en_ph * znu
                    exp_cut = 3e2
                    f_nevo  = (0. if arg_y  > exp_cut else (1. + df) / (np.exp(arg_y)  + 1.))
                    f_fd_nu = (0. if arg_nu > exp_cut else 1.             / (np.exp(arg_nu) + 1.))
