...
Design
------
Module-level state set once by ``initialise(cfg)`` (called from
``PyPR.__init__``). Multiple ``PyPR`` instances in one process
will share and overwrite this state — do not run them concurrently.
"""
...
# ---------------------------------------------------------------------------
# Module-level state (bound once by initialise)
# ---------------------------------------------------------------------------
_cfg = None

# JIT-compiled (or plain Python) e± integrand implementations.
# Each integrand is a function (E, Tg) where E = ε/T is dimensionless
# and Tg is the photon temperature in MeV.  These are set by
# _setup_integrand_impls() and used by the _*_exact() functions below.
_rho_e_int_impl  = None   # integrand for ρ_e
_drho_e_dT_impl  = None   # integrand for dρ_e/dT
_p_e_int_impl    = None   # integrand for p_e
_dp_e_dT_impl    = None   # integrand for dp_e/dT

# QED interaction-pressure correction callables (set by _load_tables).
# δP(T) encodes the leading finite-temperature QED correction to the EM
# plasma pressure; ρ_QED = T dδP/dT − δP,  p_QED = δP  (Phys. Rep. §II.E).
PQEDofT   = None   # δP(Tγ) [MeV⁴]
dPQEDdT   = None   # d(δP)/dTγ [MeV³]
d2PQEDdT2 = None   # d²(δP)/dTγ² [MeV²]

# Electron-thermodynamics interpolants (set by _build_electron_tables when
# cfg.tabulate_electron_thermo is True; left as None to fall through to the
# exact quad-based implementations).
_rho_e_tab     = None
_p_e_tab       = None
_drho_e_dT_tab = None
_dp_e_dT_tab   = None
...
def initialise(cfg):
    """Bind this module to a ``PyPRConfig`` instance and load all tables.

    Must be called once before any thermodynamic function is used.
    Internally calls ``_load_tables`` (QED pressure correction files),
    ``_setup_integrand_impls`` (compile/define the e± integrands),
    and ``_build_electron_tables`` (optional pre-tabulation of e±).
...
def _load_tables(cfg):
    ...
    The three module-level callables ``PQEDofT``, ``dPQEDdT``, ``d2PQEDdT2``
    represent the finite-temperature QED correction to the EM plasma
    ...
    **Recompute mode** (``recompute_qed_corrections=True``):
    ...
    if recompute or not files_present:
        ...
        PQEDofT   = lambda T, _s=spl_P:   float(_s(T))
        dPQEDdT   = lambda T, _s=spl_dP:  float(_s(T))
        d2PQEDdT2 = lambda T, _s=spl_d2P: float(_s(T))
        return

    # Load from the saved files (linear interpolation matches the file precision).
    t = np.loadtxt(p_file)
    PQEDofT = interp1d(t[:, 0], t[:, 1] + t[:, 2], bounds_error=False,
                    fill_value="extrapolate", assume_sorted=False, kind='linear')
    t = np.loadtxt(dp_file)
    dPQEDdT = interp1d(t[:, 0], t[:, 1] + t[:, 2], bounds_error=False,
                    fill_value="extrapolate", assume_sorted=False, kind='linear')
    t = np.loadtxt(d2p_file)
    d2PQEDdT2 = interp1d(t[:, 0], t[:, 1] + t[:, 2], bounds_error=False,
                      fill_value="extrapolate", assume_sorted=False, kind='linear')
...
def _setup_integrand_impls(cfg):
    ...
    if cfg.numba_installed:
        try:
            ...
            _rho_e_int_impl = _rho_e_intgd
            _drho_e_dT_impl = _drho_e_dT_intgd
            _p_e_int_impl   = _p_e_intgd
            _dp_e_dT_impl   = _dp_e_dT_intgd
            return
        except ImportError:
            pass
...
# ---------------------------------------------------------------------------
# Each of the four quantities has an ``_*_exact`` implementation using
...
_ELECTRON_CACHE_PATH = None   # set once by the first call to _build_electron_tables


def _build_electron_tables(cfg):
    ...
    """
    global _rho_e_tab, _p_e_tab, _drho_e_dT_tab, _dp_e_dT_tab, _ELECTRON_CACHE_PATH
    ...
    if not cfg.recompute_electron_thermo and os.path.exists(cache_path):
        try:
            ...
            _p_e_tab       = interp1d(d[:, 0], d[:, 2], kind='cubic',
                                      bounds_error=False, fill_value="extrapolate")
            _drho_e_dT_tab = interp1d(d[:, 0], d[:, 3], kind='cubic',
                                      bounds_error=False, fill_value="extrapolate")
            _dp_e_dT_tab   = interp1d(d[:, 0], d[:, 4], kind='cubic',
                                      bounds_error=False, fill_value="extrapolate")
            if cfg.verbose:
    ...
    rho_e_arr     = np.array([_rho_e_exact(T)     for T in grid])
    p_e_arr       = np.array([_p_e_exact(T)       for T in grid])
    drho_e_dT_arr = np.array([_drho_e_dT_exact(T) for T in grid])
    dp_e_dT_arr   = np.array([_dp_e_dT_exact(T)   for T in grid])

    _rho_e_tab     = interp1d(grid, rho_e_arr,     kind='cubic',
                              bounds_error=False, fill_value="extrapolate")
    _p_e_tab       = interp1d(grid, p_e_arr,       kind='cubic',
                              bounds_error=False, fill_value="extrapolate")
    _drho_e_dT_tab = interp1d(grid, drho_e_dT_arr, kind='cubic',
                              bounds_error=False, fill_value="extrapolate")
    _dp_e_dT_tab   = interp1d(grid, dp_e_dT_arr,   kind='cubic',
                              bounds_error=False, fill_value="extrapolate")
...
def rho_e(Tg):
    ...
    Example
    -------
    >>> rho_e(0.511)   # at Tγ ≃ me, e± contribution is comparable to photons
    """
    if _rho_e_tab is None:
        return _rho_e_exact(Tg)
    if Tg < _cfg.me / _ELEC_THERMO_LOWT_RATIO:
        return 0.0
    return float(_rho_e_tab(Tg))
...
def drho_e_dT(Tg):
    ...
    float
        dρ_e/dTγ in MeV³.
    """
    if _drho_e_dT_tab is None:
        return _drho_e_dT_exact(Tg)
    if Tg < _cfg.me / _ELEC_THERMO_LOWT_RATIO:
        return 0.0
    return float(_drho_e_dT_tab(Tg))
...
def p_e(Tg):
    ...
    """
    if _p_e_tab is None:
        return _p_e_exact(Tg)
    if Tg < _cfg.me / _ELEC_THERMO_LOWT_RATIO:
        return 0.0
    return float(_p_e_tab(Tg))
...
def dp_e_dT(Tg):
    ...
    float
        dp_e/dTγ in MeV³.
    """
    if _dp_e_dT_tab is None:
        return _dp_e_dT_exact(Tg)
    if Tg < _cfg.me / _ELEC_THERMO_LOWT_RATIO:
        return 0.0
    return float(_dp_e_dT_tab(Tg))
...
def T_nu_decoupling(Tg):
    ...
    **QED-correction caveat**: when ``cfg.QED_corrections`` is True,
    ...
    is inconsistent.  This function is self-consistent only when
    ...
    This function is used by ``rho_nu_extra`` (extra ΔNeff species) and
    ...
    Example
    -------
    >>> T_nu_decoupling(0.01)   # well after e± annihilation
...
def rho_nu_extra(Tg):
    ...
    SM neutrino temperature ``T_nu_decoupling``).  Returns 0 when
    ``cfg.DeltaNeff == 0``.
...
def rho_SM(Tg, Tnue, Tnumu):
    ...
    Example
    -------
    >>> rho_SM(1.0, 1.0, 1.0)   # all temperatures equal at 1 MeV
    """
    rho_qed = Tg * dPQEDdT(Tg) - PQEDofT(Tg)   # QED correction: T dδP/dT − δP
    return (rho_g(Tg) + rho_e(Tg) + rho_qed
...
def p_SM(Tg, Tnue, Tnumu):
    ...
    float
        p_SM in MeV⁴.
    """
    return (rho_g(Tg) / 3. + p_e(Tg) + PQEDofT(Tg)
...
def spl(Tg):
    ...
    Example
    -------
    >>> spl(10.0) / (11 * np.pi**2 / 45 * 10.**3)   # should be ≈ 1 at high T
    0.9999...
    """
    rho_pl = rho_g(Tg) + rho_e(Tg)
    p_pl   = rho_g(Tg) / 3. + p_e(Tg)
    rho_qed = Tg * dPQEDdT(Tg) - PQEDofT(Tg)   # QED correction to energy density
    p_qed   = PQEDofT(Tg)                    # QED correction to pressure
...
def spl_and_dspl_dT(Tg):
    ...
    Example
    -------
    >>> s, dsdT = spl_and_dspl_dT(1.0)
    """
    rho_g_val  = rho_g(Tg)
    rho_e_val  = rho_e(Tg)
    p_e_val    = p_e(Tg)
    PQEDofT_val   = PQEDofT(Tg)
    dPQEDdT_val   = dPQEDdT(Tg)
    d2PQEDdT2_val = d2PQEDdT2(Tg)
    ...
    drho_pl_dT    = drho_g_val + drho_e_dT(Tg)
    dp_pl_dT      = drho_g_val / 3. + dp_e_dT(Tg)
...
def dspl_dT(Tg):
    ...
    Delegates to ``spl_and_dspl_dT`` and discards the entropy value.
    Use ``spl_and_dspl_dT`` directly when both s and ds/dT are needed.
